...
<title>Everest &amp; Fiovivor Archive</title>
<meta property="og:title" content="Everest &amp; Fiovivor Archive">
<meta property="og:description" content="Discord channel archives for Fiovivor &amp; Everest">
...
:root{
  ...
  --accent:#6cc24a; --accent-soft:rgba(108,194,74,.15);
}
...
.world{width:100vw;height:100vh;overflow-y:auto;overflow-x:hidden;position:relative;scroll-behavior:smooth}
...
.tagline{max-width:640px;color:#cdd9ee;font-size:1.12rem;line-height:1.6;margin-top:1.7rem;
...
.discord-btn{margin-top:1.8rem;display:inline-flex;align-items:center;gap:.55rem;
  background:var(--accent);color:#0a2110;font-weight:800;font-size:.98rem;letter-spacing:.01em;
  padding:.8rem 1.5rem;border-radius:100px;transition:transform .18s,filter .18s,box-shadow .18s;
  box-shadow:0 10px 30px rgba(108,194,74,.28);animation:fade 1s .85s both}
.discord-btn:hover{transform:translateY(-2px);filter:brightness(1.06);box-shadow:0 14px 38px rgba(108,194,74,.42)}
...
@media(max-width:600px){
  ...
  .flag{font-size:.58rem;padding:.8rem .6rem}
  .tagline{font-size:1rem}
  .wordmark{font-size:clamp(3.4rem,22vw,8rem)}
}
...
function showFio(){ track.classList.add('show-fio'); toFio.style.display='none'; toEverest.style.display='flex'; snowEl.style.opacity='0'; }
function showEverest(){ track.classList.remove('show-fio'); toEverest.style.display='none'; toFio.style.display='flex'; snowEl.style.opacity='1'; }
